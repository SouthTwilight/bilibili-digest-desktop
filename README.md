...
- **AI 总结文档**（导出库）：将 MD 学习资料深度提炼为四层结构化总结（快速概览 + 主题深度分析 + 关键原话 + 全时间线覆盖 + 行动项）；超长文档（如全部P合并导出）自动分块总结再汇总，不再截断
...
- **单视频 / 合集导出**：Markdown / HTML 学习资料（含 AI 章节）
  ...
  - **多P视频**：「导出全部P」将所有分P合并为一份文档（每个P一个章节、时间戳直达对应分P）；合集导出自动包含多P视频的全部分P
...
└─ 合集名/                    ← 视频在合集中时
   └─ 视频名_BV号/
      ...
      ├─ AI总结_视频名.md          ← AI 总结文档
      ├─ 视频名_日期时间.md        ← 字幕/学习资料导出（多P视频：当前P为 视频名_P2_日期时间.md，全部P合并为 视频名_全部P_日期时间.md）
      ├─ notes.json               ← 笔记数据
      └─ picture/                  ← 笔记截图（自动存入）
```
